...
function resolveConfig(pluginConfig) {
    ...
    return {
        reflectionDepth: Math.min(3, Math.max(1, cfg.reflectionDepth || 2)),
        reasoningMode: cfg.reasoningMode || "adaptive",
        entropyThreshold: Math.min(1, Math.max(0, cfg.entropyThreshold || 0.7)),
        autoContemplate: cfg.autoContemplate ?? true,
        knowledgeGraphEnabled: cfg.knowledgeGraphEnabled ?? true,
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        api.registerTool({
            name: "cortex_reflect",
            label: "Reflect",
            description: "Perform structured self-reflection on a thought, response, or plan. " +
        ...
        api.registerTool({
            name: "cortex_reason",
            label: "Reason",
            description: "Build a structured reasoning chain for a query. Detects logical fallacies, " +
            ...
            async execute(_id, params) {
                const mode = params.mode || config.reasoningMode;
        ...
        api.registerTool({
            ...
            label: "Synthesize",
            description: "Synthesize multiple information sources into a unified insight. " +
        ...
        api.registerTool({
            ...
            label: "Contemplate",
            description: "Deep contemplation of a complex inquiry through multiple passes: " +
        ...
        api.registerTool({
            name: "cortex_status",
            label: "Status",
            description: "Get current cognitive status: entropy level, knowledge graph stats, " +
            ...
            async execute(_id, _params) {
                ...
                lines.push(``, `## Trend`, `- Direction: ${trend.increasing ? "📈 Increasing" : "📉 Stable/Decreasing"}`, `- Average: ${trend.average}`, `- Peak: ${trend.peak}`, ``, `## Knowledge Graph`, `- Nodes: ${graph.nodes}`, `- Edges: ${graph.edges}`);
