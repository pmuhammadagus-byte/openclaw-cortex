 ...
 */
export function checkEntropy(options) {
 ...
 */
export function getEntropyTrend(sessionId) {
...
function calculateTopicDrift(messages) {
    ...
    for (let i = 2; i < messages.length; i++) {
        const hasTopicShift = topicWords.some((w) => messages[i].toLowerCase().includes(w) && !messages[i - 1].toLowerCase().includes(w));
        if (hasTopicShift)
            driftCount++;
    }
    return Math.min(1.0, driftCount / messages.length);
}
function detectAnomalies(messages) {
    let count = 0;
