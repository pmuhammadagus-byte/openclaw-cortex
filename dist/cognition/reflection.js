...
function critiqueFactualAccuracy(thought, _context) {
    const issues = [];
...
function critiqueCompleteness(thought, _context) {
    const issues = [];
...
function critiqueBiasAndAssumptions(thought, _context) {
    const issues = [];
...
function applyFixes(thought, issues) {
    if (issues.length === 0)
        return thought;
