...
import type { KnowledgeGraph, KnowledgeNode } from "../utils/types.js";
 ...
 */
export function extractAndStore(options: ExtractOptions): KnowledgeGraph {
  ...
  for (const rel of relations) {
    ...
    if (sourceNode && targetNode) {
      const existing = graph.edges.find(
        (e) => e.source === sourceNode.id && e.target === targetNode.id && e.relation === rel.relation
      );
      if (existing) {
 ...
 */
export function queryGraph(entity: string, depth: number = 1): { node: KnowledgeNode | null; related: KnowledgeNode[] } {
 ...
 */
export function getGraphStats(): { nodes: number; edges: number; topEntities: KnowledgeNode[] } {
...
function extractRelations(text: string, entities: string[]): Array<{ source: string; target: string; relation: string }> {
...
function inferEntityType(entity: string, context: string): KnowledgeNode["type"] {
