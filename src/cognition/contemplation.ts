...
import type { ContemplationResult, ContemplationPass } from "../utils/types.js";
 ...
 */
export async function contemplate(options: ContemplateOptions): Promise<ContemplationResult> {
  const { inquiry, passes = 3, context } = options;

  const contemplationPasses: ContemplationPass[] = [];
...
async function explorePhase(inquiry: string, _context?: string): Promise<ContemplationPass> {
  const insights = [
...
async function reflectPhase(inquiry: string, previous: ContemplationPass, _context?: string): Promise<ContemplationPass> {
  const insights = [
...
async function synthesizePhase(inquiry: string, previousPasses: ContemplationPass[], _context?: string): Promise<ContemplationPass> {
  const insights = [
...
function generateFinalSynthesis(inquiry: string, passes: ContemplationPass[]): string {
...
function calculateCertainty(passes: ContemplationPass[]): number {
...
function extractOpenQuestions(insights: string[]): string[] {
